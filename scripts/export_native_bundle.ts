...
const iosAssets = join(root, "ios", "CulinaryAlchemy", "Resources", "game");
const androidAssets = join(root, "android", "app", "src", "main", "assets", "game");
...
writeFileSync(join(iosAssets, "game_bundle.json"), bundleJson);
writeFileSync(join(androidAssets, "game_bundle.json"), bundleJson);
...
writeFileSync(join(iosAssets, "transitions.json"), transitionsJson);
writeFileSync(join(androidAssets, "transitions.json"), transitionsJson);
...
console.log(`→ core/assets, ios, android, web/src/public/game`);
