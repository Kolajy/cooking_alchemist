...
def generate():
    # 1. Truncate previous added properties to keep properties.ts clean
    properties_path = "content/data/ingredients/properties.ts"
    with open(properties_path, "r") as f:
        content = f.read()

    # Clear previously added parts starting from "pork" or "livestock"
    pork_index = content.find("  pork: ")
    if pork_index != -1:
        content = content[:pork_index] + "};\n"
    livestock_index = content.find("  livestock: ")
    if livestock_index != -1:
        content = content[:livestock_index] + "};\n"
    ash_cake_index = content.find("  ash_cake: ")
    if ash_cake_index != -1:
        content = content[:ash_cake_index] + "};\n"

    # We want to re-add the stone age recipes properties first to be consistent
    stone_age_props = {
    ...
    # Add stone age items
    for item, prop in stone_age_props.items():
